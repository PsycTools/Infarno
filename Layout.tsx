...
import { Flame, Ghost, Menu, X, ArrowRight, Shield, HelpCircle, FileText, Mail } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="min-h-screen relative flex flex-col font-sans text-gray-200 selection:bg-demon-neon selection:text-white pb-20">
      ...
      {/* --- MOBILE MENU OVERLAY --- */}
      <div className={`fixed inset-0 z-40 bg-demon-black/95 backdrop-blur-xl transition-all duration-300 ease-in-out md:hidden flex flex-col pt-24 px-6
          ...
          <div className="flex flex-col h-full overflow-y-auto pb-10">
              ...
              <div className="flex flex-col gap-4 mb-8">
                  <button 
                    ...
                    className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 text-lg font-bold font-brand tracking-wider hover:bg-white/10 hover:border-demon-neon/50 transition-all group"
                  >
                    <span>ARSENAL</span>
                    <ArrowRight className="w-5 h-5 text-gray-500 group-hover:text-demon-neon group-hover:translate-x-1 transition-all" />
                  </button>
                  ...
                  <button className="p-4 rounded-xl bg-demon-blood/20 border border-demon-blood/50 text-lg font-bold font-brand tracking-wider text-demon-neon shadow-[0_0_20px_rgba(139,0,0,0.2)] mt-2">
                    GET PREMIUM ACESS
                  </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <button onClick={() => handleMobileNav(onNavigateFAQ)} className="flex items-center gap-3 text-gray-400 hover:text-white p-2">
                    <HelpCircle className="w-4 h-4" /> <span className="text-sm">FAQ</span>
                 </button>
                 <button onClick={() => handleMobileNav(onNavigatePrivacy)} className="flex items-center gap-3 text-gray-400 hover:text-white p-2">
                    <Shield className="w-4 h-4" /> <span className="text-sm">Privacy</span>
                 </button>
                 <button onClick={() => handleMobileNav(onNavigateTerms)} className="flex items-center gap-3 text-gray-400 hover:text-white p-2">
                    <FileText className="w-4 h-4" /> <span className="text-sm">Terms</span>
                 </button>
                 <button onClick={() => handleMobileNav(onNavigateContact)} className="flex items-center gap-3 text-gray-400 hover:text-white p-2">
                    <Mail className="w-4 h-4" /> <span className="text-sm">Contact</span>
                 </button>
              </div>

              {/* Decorative Bottom */}
              <div className="mt-auto text-center pt-10 pb-6 opacity-40">
                <Flame className="w-8 h-8 mx-auto text-demon-neon mb-2" />
                <p className="text-[10px] uppercase tracking-widest text-gray-500">Infrano Tools Mobile</p>
              </div>
          </div>
      </div>
      ...
      <footer className="border-t border-demon-blood/20 bg-black/80 backdrop-blur-md mt-12 py-10 pb-32 md:pb-10">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center justify-center text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-4 text-xs text-gray-500">
             ...
             <span className="hidden md:inline">•</span>
             <button onClick={onNavigateTerms} className="hover:text-demon-neon transition-colors">Terms</button>
             <span className="hidden md:inline">•</span>
             <button onClick={onNavigateContact} className="hover:text-demon-neon transition-colors">Contact</button>
             <span className="hidden md:inline">•</span>
             <button onClick={onNavigatePrivacy} className="hover:text-demon-neon transition-colors">Privacy</button>
          </div>
          
          <div className="mt-8 pt-6 border-t border-gray-900/50 w-full max-w-md text-center">
             <p className="text-[10px] text-gray-600 font-mono tracking-widest uppercase">
               © 2025 Infrano Tools. All Rights Reserved.
             </p>
          </div>
        </div>
      </footer>
